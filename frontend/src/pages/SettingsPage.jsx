...
import { evolutionAPI, healthAPI, riskAPI, runtimeAPI } from "@/lib/api";
...
export default function SettingsPage() {
  ...
  const fetchRuntime = useCallback(async () => {
    ...
    setError(null);
    try {
      const [healthResponse, riskResponse, evolutionResponse, runtimeResponse, sessionsResponse] = await Promise.all([
        healthAPI.health(), riskAPI.activeProfile(), evolutionAPI.activePolicy(), runtimeAPI.status(), runtimeAPI.sessions({ limit: 20 }),
      ]);
      ...
      setRuntime(null); setRiskProfile(null); setEvolutionPolicy(null); setRuntimeStatus(null); setRuntimeSessions([]);
    ...
    <div className="min-h-screen bg-background" data-testid="settings-page">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="glass-card rounded-xl overflow-hidden"><div className="px-5 py-3 border-b border-white/5 flex items-center gap-2"><Database className="w-4 h-4 text-cyan-400" /><h2 className="evo-section-title">Runtime efectivo</h2></div><div className="px-5">
          ...
          <RuntimeRow label="Trading automático" value={runtime?.automated_trading || "unknown"} description="Disponible únicamente dentro de sesiones Paper Phase 7 explícitamente iniciadas; no implica Live." />
        ...
        <div className="glass-card rounded-xl overflow-hidden"><div className="px-5 py-3 border-b border-white/5 flex items-center gap-2"><Activity className="w-4 h-4 text-cyan-400" /><h2 className="evo-section-title">Controles disponibles</h2></div><div className="px-5 py-4 text-sm text-muted-foreground space-y-2">
          ...
          <p>La replicación Phase 6 sigue siendo manual: el runtime 24/7 no replica agentes automáticamente.</p>
        </div></div>

        <div className="glass-card rounded-xl p-5 border border-amber-500/15"><div className="flex items-start gap-3"><ShieldAlert className="w-5 h-5 text-amber-400 mt-0.5 shrink-0" /><div><p className="text-sm font-medium text-foreground">Live continúa estructuralmente aislado</p><p className="text-xs text-muted-foreground mt-1">MongoDB y motores legacy siguen fuera del runtime activo. Phase 7 automatiza únicamente Paper con capital virtual; no existe adaptador capaz de enviar órdenes reales.</p></div></div></div>
      </div>
    </div>
  );
}
