...
Handles all database operations with the new schema
"""
...
from models import (
    ...
    Strategy, Trade, TradeStatus, TradeSide, TradeCreateRequest,
    Position, Wallet, AssetBalance, RiskProfile,
    Signal, MarketData, AgentLineage, LineageTreeNode, LineageStats,
    AuditLog, AuditActor, AuditTarget, OrchestratorState,
    Lineage, Lifecycle, AgentFinances, Performance, TradingStats,
    AgentConfig, ReplicationRules, DeathRules, AgentMetadata,
    TradeEntry, TradeResult, RiskManagement
)
...
class DatabaseService:
    ...
    async def _log_audit(
        ...
        log = AuditLog(
            ...
            target=AuditTarget(type=target_type, id=target_id, name=target_name) if target_id else None,
            details=details or {},
            result=result,
    ...
    async def create_agent(self, request: AgentCreateRequest, created_by: str = "user") -> Agent:
        """Create a new agent with full schema"""
        
        # Create agent with all nested structures
        agent = Agent(
            name=request.name,
            ...
            generation=1,
            lineage=Lineage(
                root_ancestor_id=None  # Will be set to self
            ),
            ...
            metadata=AgentMetadata(
                ...
                tags=[request.agent_type.value]
            )
        )
        
        # Set root ancestor to self for original agents
        agent.lineage.root_ancestor_id = agent.id
        
        # If parent specified, update lineage
        if request.parent_id:
            parent = await self.get_agent(request.parent_id)
            if parent:
                agent.lineage.parent_id = request.parent_id
                agent.lineage.root_ancestor_id = parent.get('lineage', {}).get('root_ancestor_id', request.parent_id)
    ...
    async def get_agents(self, status: Optional[AgentStatus] = None, limit: int = 100) -> List[Dict]:
    ...
    async def update_agent(self, agent_id: str, updates: Dict) -> bool:
        ...
        result = await self.agents.update_one(
            {"id": agent_id},
            {"$set": self._serialize_datetime(updates)}
        )
        return result.modified_count > 0
    
    async def replicate_agent(self, agent_id: str, request: AgentReplicateRequest) -> Optional[Agent]:
        ...
        min_balance_for_replication = parent.get('replication_rules', {}).get('min_balance_usd', 50) * 2
        
        if parent_balance < min_balance_for_replication:
            raise ValueError(f"Insufficient balance for replication. Need at least {min_balance_for_replication}")
        ...
        child_name = request.custom_name or f"{parent['name']}_child_{children_count + 1}"
        ...
        if request.mutation_enabled and request.mutation_rate > 0:
    ...
    async def update_agent_status(self, agent_id: str, status: AgentStatus, reason: str = None):
        ...
        await self._log_audit(
            ...
            target_type="agent",
            target_id=agent_id,
            target_name=agent.get('name', 'Unknown'),
    ...
    async def simulate_trade(self, agent_id: str, profit: float) -> Dict:
        ...
        # Calculate ROI
        roi = ((new_balance - initial_capital) / initial_capital) * 100 if initial_capital > 0 else 0
        ...
        elif roi >= replication_rules.get('min_roi_to_replicate', 50) and new_trades >= replication_rules.get('min_trades_to_replicate', 100):
        ...
        elif not is_winner and abs(current_streak) > stats.get('consecutive_losses_max', 0):
        ...
        trade = Trade(
            ...
            entry=TradeEntry(
                price=42000,
                quantity=abs(profit) / 42000,
                value_usd=abs(profit)
            ),
            result=TradeResult(
                pnl_usd=profit,
                pnl_percent=(profit / current_balance * 100) if current_balance > 0 else 0,
                net_pnl_usd=profit,
                is_winner=is_winner
            ),
            status=TradeStatus.CLOSED
        )
        trade_doc = self._serialize_datetime(trade.model_dump())
    ...
    async def create_wallet(self, agent_id: str, initial_balance: float = 0) -> Wallet:
        ...
        doc = self._serialize_datetime(wallet.model_dump())
        await self.wallets.insert_one(doc)
        return wallet
    ...
    async def create_trade(self, request: TradeCreateRequest) -> Trade:
        ...
        trade = Trade(
            ...
            quote_asset=request.symbol.split('/')[1] if '/' in request.symbol else 'USDT',
            side=request.side,
            strategy_id=request.strategy_id,
            entry=TradeEntry(
        ...
        doc = self._serialize_datetime(trade.model_dump())
    ...
    async def get_all_trades(self, limit: int = 100) -> List[Dict]:
        ...
        return await self.trades.find({}, {"_id": 0}).sort("created_at", -1).to_list(limit)
    
    # ==================== STRATEGIES ====================
    
    async def create_strategy(self, name: str, description: str = "", **kwargs) -> Strategy:
        ...
        doc = self._serialize_datetime(strategy.model_dump())
        await self.strategies.insert_one(doc)
        return strategy
    ...
    async def create_risk_profile(self, name: str, description: str = "", **kwargs) -> RiskProfile:
        ...
        doc = self._serialize_datetime(profile.model_dump())
        await self.risk_profiles.insert_one(doc)
        return profile
    ...
    async def get_orchestrator_state(self) -> Dict:
        ...
        state = await self.orchestrator_state.find_one({"orchestrator_id": "main"}, {"_id": 0})
    ...
    async def update_orchestrator_metrics(self):
        ...
        total_capital = sum(a.get('finances', {}).get('current_balance', 0) for a in agents)
    ...
    async def rebuild_lineage_tree(self, root_agent_id: str):
        ...
        all_agents = await self.agents.find(
            {"lineage.root_ancestor_id": root_agent_id},
            {"_id": 0}
        ).to_list(1000)
        ...
        stats = {
            ...
            "replicating_agents": len([a for a in all_agents if a.get('status') == 'replicating']),
            "max_generation": max((a.get('generation', 1) for a in all_agents), default=1),
            "total_capital_managed": sum(a.get('finances', {}).get('current_balance', 0) for a in all_agents),
            "survival_rate": len([a for a in all_agents if a.get('status') != 'dead']) / len(all_agents) if all_agents else 0
        }
    ...
    async def get_dashboard_stats(self) -> Dict:
        ...
        dying = [a for a in agents if a.get('status') == 'dying']
        dead = [a for a in agents if a.get('status') == 'dead']
        replicating = [a for a in agents if a.get('status') == 'replicating']
        
        # Financial stats
        total_balance = sum(a.get('finances', {}).get('current_balance', 0) for a in agents)
        total_initial = sum(a.get('finances', {}).get('initial_capital', 0) for a in agents)
        avg_roi = sum(a.get('performance', {}).get('roi_percent', 0) for a in agents) / len(agents) if agents else 0
        
        # Trading stats
        winning_trades = [t for t in trades if t.get('result', {}).get('is_winner', False)]
        total_pnl = sum(t.get('result', {}).get('pnl_usd', 0) for t in trades)
        
        # Top performers
        sorted_by_roi = sorted(agents, key=lambda a: a.get('performance', {}).get('roi_percent', 0), reverse=True)
        ...
        return {
            "agents": {
                ...
                "by_type": {
                    "crypto_trader": len([a for a in agents if a.get('agent_type') == 'crypto_trader']),
                    "business_scout": len([a for a in agents if a.get('agent_type') == 'business_scout']),
                    "market_analyzer": len([a for a in agents if a.get('agent_type') == 'market_analyzer'])
                }
            },
            "finances": {
            ...
            "lineage": {
                "total_generations": max((a.get('generation', 1) for a in agents), default=1),
                "total_replications": sum(a.get('lineage', {}).get('clone_count', 0) for a in agents)
            }
        }
