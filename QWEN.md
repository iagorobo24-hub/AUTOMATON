...
- No `/api/live/orders`, buy/sell, credential-write or activation endpoint may exist.
- No real exchange secrets are stored.
- `live-v1` enforces conservative venue/hard-limit readiness checks.
- Prepared Live intents are deterministic/idempotent records only; they are never transmitted.
- Intent preparation requires a promoted StrategyCandidate, matching current source SHA and prior `ARCHITECTURE_READY` evaluation.
- Market Data must be real/fail-closed and non-executing.
- Risk must be active and Paper recovery clean.
- Live reconciliation ambiguity => `RECOVERY_REQUIRED` + circuit breaker; never replay.
- Recovery remains blocking until the operator explicitly resolves the exact record with a reason.
- Persistent emergency stop blocks preparation and cannot clear over unresolved recovery.
- Every readiness evaluation preserves `real_capital_blocked=true`.
...
A concrete exchange adapter, secret-management mechanism and any real-capital activation require a new explicitly authorized scope after venue-specific integration/recovery tests and operational review. Do not revive deleted legacy Binance/TradingEngine code.
